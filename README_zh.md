...
- **📜 优化历史**: 按模型自动保存优化历史，支持一键恢复与回顾。
- **☁️ 云端同步**: 使用 GitHub Gist 跨设备同步优化历史（需具有 `gist` 权限的 GitHub PAT）。
...
- **本地存储**: 所有数据（API 密钥、设置）仅存储在您浏览器的 `localStorage` 中，永不发送到任何服务器。
- **无后端服务**: 这是纯静态 SPA 应用，没有后端服务器收集或存储您的数据。
- **直连 API**: API 请求从您的浏览器直接发送到 OpenAI/Anthropic/Google，无中间代理。
- **用户隔离**: 每个用户的数据完全隔离，同一部署域名的其他用户无法看到您的数据。
- **持久存储**: 设置将持久保存，直到您手动清除浏览器数据。无需每次访问都重新输入 API 密钥。
